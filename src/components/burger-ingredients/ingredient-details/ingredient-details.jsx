...
const IngredientDetails = () => {
    ...
    let {ingredient: stateIngredient} = useSelector(
        state => state.currentIngredient
    );
    const [ingredient, setIngredient] = useState(stateIngredient);

    if (typeof ingredient === 'object' && (Object.keys(ingredient).length === 0)) {
        getIngredientsRequest()
        .then(res => {
            setIngredient(res.data.find(item => item._id === location.pathname.split('/')[2])) 
        } )
    ...
    return (
        ...
        {typeof ingredient === 'object' && (Object.keys(ingredient).length !== 0) &&
            (   <> 
            {!(location.state && location.state.background) ? (<div className={styles.head}>
